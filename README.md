...
| `group_kick_user` | group_id, user_id, reason | **仅群主** | 踢出用户。管理员踢人前需征求群主同意 |
...
| `group_post_notice` | group_id, content | **仅群主** | 发布群公告，返回 notice_id 供后续删除 |
| `group_delete_notice` | group_id, notice_id | **仅群主** | 删除群公告（先用 group_get_notice 获取 notice_id） |

#### 查询（4 个）

| Tool | 参数 | 说明 |
|------|------|------|
| `group_get_member` | group_id, user_id | 查询群成员身份（owner/admin/member）、昵称和群名片。踢人/禁言前必须先调用 |
| `group_get_shut_list` | group_id | 查看当前群的禁言列表 |
| `group_get_system_msg` | group_id | 获取群系统消息（入群申请、邀请入群） |
| `group_get_notice` | group_id | 获取群公告列表（含 notice_id），删除公告前调用 |
